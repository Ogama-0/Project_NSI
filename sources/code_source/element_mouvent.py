...
class sprite_mouvent_screen :
    def __init__(self, nom : str, nombre_de_sprite : int , screen : pygame.surface.Surface , position : list[int], frame_decalage : int):
        ...
        self.screen = screen
        self.image = pygame.transform.scale(pygame.image.load(self.path),screen.get_size())
    ...
    def image_suivante(self) :
        ...
        if self.frame > 1000000 :
            self.frame = 0
        print(f"curent = {self.curent_image}")
